    ...
    if note_condition:
        for i in range(len(stems)):
            ...
            if head_exist or hat_exist:
                ...
                note_classification = (
                    ((hat_exist and not head_fill and tail_cnt == 0 and not dot_exist), 4),
...
def recognize_pitch(image, staff, head_center):
    pitch_lines = [staff[4] + fs.weighted(5) - fs.weighted(5) * i for i in range(15)]
...
def recognize_rest(image, staff, stats):
    ...
    if rest_condition:
        ...
        elif fs.weighted(25) >= h >= fs.weighted(23):
            if fs.weighted(14) >= w >= fs.weighted(12):
                rest = 8
        elif fs.weighted(9) >= h:
            if staff[1] + fs.weighted(5) >= center >= staff[0]:
                rest = 1
